...
const event_typesTable = db['event_types'];
    ...
    event_typesTable.findAll()
        ...
        .then(result => {
            if (result.length === 0) {
                ...
                res.json({Message : "Aucun event_type présent en base de données."})
            } else {
        ...
        .catch(error => {
            ...
            const message = "La liste des type d'events n'a pas pu être récupérée. Réessayez dans quelques instants."
            res.status(500).json({
                message,
                data: error
            })
...
const getOneById = (req, res) => {
    event_typesTable.findByPk(req.params.id)
        .then(event => {
            if (!event) {
                return res.status(404).json({ message: "Aucun type d'event n'a été trouvé" })
            }
            res.status(200).json(event)
        })
        .catch(error => {
            const message = "Une erreur a eu lieu lors de la récupération du type d'event."
            res.status(500).json({
                message,
                data: error
            })
...
const createOne = (req, res) => {
    event_typesTable.create(req.body)

        .then(event => {
            const message = "Type d'event ajouté à la base de données."
            res.status(201).json({
                message,
                data: event
            })
...
const updateOneById = (req, res) => {
    event_typesTable.findByPk(req.params.id)
        .then(event => {
            if(!event) {
                return res.status(404).json({message: "Aucune type d'event n'a été trouvé"})
            }
        
            event_typesTable.update(
                req.body,
                    {
                    where: {
                        id: req.params.id
                ...
                .then(result => {
                    const message = "Type d'event correctement mise à jour."
                    res.status(201).json({
                        message
                    });
                })
...
module.exports = event_typeController