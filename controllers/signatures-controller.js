...
//On initialise une nouvelle constante qui représente le modèle qui nous intéresse. Ici, la table signatures
const signaturesTable = db['signatures'];
    ...
    signaturesTable.findAll()
        ...
        .then(result => {
            if (result.length === 0) {
                ...
                res.json({Message : "Aucun signature présent en base de données."})
            } else {
        ...
        .catch(error => {
            ...
            const message = "La liste des signatures n'a pas pu être récupérée. Réessayez dans quelques instants."
            res.status(500).json({
                message,
                data: error
            })
...
const getOneById = (req, res) => {
    signaturesTable.findByPk(req.params.id)
        .then(signature => {
            if (!signature) {
                return res.status(404).json({ message: "Aucun signature n'a été trouvé" })
            }
            res.status(200).json(signature)
        })
        .catch(error => {
            const message = "Une erreur a eu lieu lors de la récupération du signature."
            res.status(500).json({
                message,
                data: error
            })
...
const createOne = (req, res) => {
    signaturesTable.create(req.body)

        .then(signature => {
            const message = "signature ajouté à la base de données."
            res.status(201).json({
                message,
                data: signature
            })
    ...
    signaturesTable.findByPk(req.params.id)
        .then(signature => {
            ...
            signaturesTable.update(
                ...
                .then(result => {
                    const message = "signature correctement mis à jour."
                    res.status(201).json({
                        message
                    });
                })
