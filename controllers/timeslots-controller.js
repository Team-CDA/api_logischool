...
//On initialise une nouvelle constante qui représente le modèle qui nous intéresse. Ici, la table timeslots
const timeslotsTable = db['timeslots'];
    ...
    timeslotsTable.findAll()
        ...
        .then(result => {
            if (result.length === 0) {
                ...
                res.json({Message : "Aucun timeslot présent en base de données."})
            } else {
        ...
        .catch(error => {
            ...
            const message = "La liste des timeslots n'a pas pu être récupérée. Réessayez dans quelques instants."
            res.status(500).json({
                message,
                data: error
            })
...
const getOneById = (req, res) => {
    timeslotsTable.findByPk(req.params.id)
        .then(timeslot => {
            if (!timeslot) {
                return res.status(404).json({ message: "Aucun timeslot n'a été trouvé" })
            }
            res.status(200).json(timeslot)
        })
        .catch(error => {
            const message = "Une erreur a eu lieu lors de la récupération de l'timeslot."
            res.status(500).json({
                message,
                data: error
            })
...
const createOne = (req, res) => {
    timeslotsTable.create(req.body)

        .then(timeslot => {
            const message = "Timeslot ajouté à la base de données."
            res.status(201).json({
                message,
                data: timeslot
            })
...
const updateOneById = (req, res) => {
    timeslotsTable.findByPk(req.params.id)
        .then(timeslot => {
            if(!timeslot) {
                return res.status(404).json({message: "Aucune timeslot n'a été trouvé"})
            }
            ...
            timeslotsTable.update(
                ...
                .then(result => {
                    const message = "Timeslot correctement mise à jour."
                    res.status(201).json({
                        message
                    });
                })
