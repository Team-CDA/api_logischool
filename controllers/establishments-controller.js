    ...
    establishmentsTable.findAll(
        ...
        .then(result => {
            if (result.length === 0) {
                ...
                res.json({Message : "Aucun établissement présent en base de données."})
            } else {
...
const getOneById = (req, res) => {
    establishmentsTable.findByPk(req.params.id)
        .then(classe => {
            if (!classe) {
                return res.status(404).json({ message: "Aucun établissements n'a été trouvé" })
            }
            res.status(200).json(classe)
        })
        .catch(error => {
            const message = "Une erreur a eu lieu lors de la récupération de l'établissements."
            res.status(500).json({
                message,
                data: error
            })
...
const createOne = (req, res) => {
    establishmentsTable.create(req.body)
