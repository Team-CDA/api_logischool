    ...
    statusesTable.findAll()
        ...
        .then(result => {
            if (result.length === 0) {
                ...
                res.json({Message : "Aucun statut présent en base de données."})
            } else {
        ...
        .catch(error => {
            ...
            const message = "La liste des statuts n'a pas pu être récupérée. Réessayez dans quelques instants."
            res.status(500).json({
                message,
                data: error
            })
    ...
    statusesTable.findByPk(req.params.id)
        .then(status => {
            ...
            statusesTable.update(
                ...
                .then(result => {
                    const message = "Statut correctement mise à jour."
                    res.status(201).json({
                        message
                    });
                })
