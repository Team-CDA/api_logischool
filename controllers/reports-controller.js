    ...
    reportsTable.findAll()
        ...
        .then(result => {
            if (result.length === 0) {
                ...
                res.json({Message : "Aucun report présent en base de données."})
            } else {
        ...
        .catch(error => {
            ...
            const message = "La liste des reports n'a pas pu être récupérée. Réessayez dans quelques instants."
            res.status(500).json({
                message,
                data: error
            })
...
const getOneById = (req, res) => {
    reportsTable.findByPk(req.params.id)
        .then(report => {
            if (!report) {
                return res.status(404).json({ message: "Aucun report n'a été trouvé" })
            }
            res.status(200).json(report)
        })
        .catch(error => {
            const message = "Une erreur a eu lieu lors de la récupération de report."
            res.status(500).json({
                message,
                data: error
            })
