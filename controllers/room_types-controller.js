...
const room_typesTable = db['room_types'];
    ...
    room_typesTable.findAll()
        ...
        .then(result => {
            if (result.length === 0) {
                ...
                res.json({Message : "Aucun type de salle n'est présent en base de données."})
            } else {
        ...
        .catch(error => {
            ...
            const message = "La liste des types de salles n'a pas pu être récupérée. Réessayez dans quelques instants."
            res.status(500).json({
                message,
                data: error
            })
...
const getOneById = (req, res) => {
    room_typesTable.findByPk(req.params.id)
        .then(room => {
            if (!room) {
        ...
        .catch(error => {
            const message = "Une erreur a eu lieu lors de la récupération du type de salle."
            res.status(500).json({
                message,
                data: error
            })
...
const createOne = (req, res) => {
    room_typesTable.create(req.body)

        .then(room => {
            const message = "Type de salle ajouté à la base de données."
            res.status(201).json({
                message,
                data: room
            })
    ...
    room_typesTable.findByPk(req.params.id)
        .then(room => {
            ...
            room_typesTable.update(
                ...
                .then(result => {
                    const message = "Le type de salle a correctement été mis à jour."
                    res.status(201).json({
                        message
                    });
                })
                .catch(error => {
                    const message = "Une erreur a eu lieu lors de la modification du type de salle ."
                    if (error instanceof ValidationError) {
                        res.status(400).send(error.errors[0].message)
                    } else {
                        res.status(500).json({
...
module.exports = room_typesController